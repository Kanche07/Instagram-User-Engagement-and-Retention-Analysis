...
select 
   case 
        when average_session_length_minutes between 5 and 10 then 'Short Sessions'
...
order by
     case 
	    when feed_time_group = 'Qucik Feed Users' then 1
...
select content_type_preference, round(avg(eqe),2) as avg_eqe
from user_metrics
...
select preferred_content_theme, round(avg(eqe),2) as avg_eqe
from user_metrics
...
select 
     case  when notification_response_rate <= 0.25 then 'Low Response'
	       when notification_response_rate <= 0.50 then 'Moderate Response'
		   when notification_response_rate <= 0.75 then 'Active Response'
		  else 'Highly Responsive'
	end as notification_group, count(*) as total_users, round(avg(eqe),3) as avg_eqe
from user_metrics
group by notification_group
)
select * from notification_activity
order by 
    case when notification_group = 'Low Response' then 1
	     when notification_group = 'Moderate Response' then 2
		 when notification_group = 'Active Response' then 3
		 else 4
end;
...
select session_group, active_minutes_group, count(*) as total_users, round(avg(me),2) as avg_me, round(avg(eqe),2) as avg_eqe
from user_segments
...
dormancy_status AS (
    SELECT
        CASE
            WHEN session_group = 'Light Users'
                 AND notification_group = 'Low Responders'
                 AND (latest_login_date - last_login_date) > 30
            THEN 'Dormancy Risk Users'
            ELSE 'Active Users'
        END AS user_status
    FROM user_segments
    CROSS JOIN max_date
...
FROM dormancy_status
GROUP BY user_status;

-- SUPPORTING METRICS

/* Insight Objective: Calculate supporting platform health metrics that complement Engagement Quality Efficiency (EQE).
Business Question: How healthy is the overall Instagram ecosystem in terms of conversations, private interactions, 
advertisement engagement, and advertisement exposure? */
